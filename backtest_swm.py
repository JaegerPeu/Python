...
def gerar_retorno_mensal(df_cota: pd.DataFrame) -> pd.DataFrame:
    ...
    # Retorno mensal: produto dos retornos diários de cada mês
...
def main():
    ...
    with st.spinner("Carregando dados base..."):
        ...
        # ✅ NOVO: Criar coluna 'Identificador' com prioridade sobre nome_fundo
        if "Identificador" in df_prop.columns:
            ...
            # Onde Identificador vazio, usar nome_fundo
            ...
            # Fallback se não existir Identificador
        ...
        # ✅ ATUALIZAR CLASSIFICAÇÃO usando Identificador como chave principal
        ...
        # Fallback para nome_fundo se Identificador não estiver no CLASSIFICACAO_ATIVOS
    ...
    with tab_consultas:
        ...
        with st.expander("Retornos do Portfolio (Backtest)", expanded=False):
            ...
            portfolios_all = [col for col in df_prop_class.columns if col not in ["Identificador", "nome_fundo", "Grande Classe", "Classe"]]
            ...
            # ✅ USAR Identificador para matching
    ...
    # LÂMINAS
    with tab_laminas:
        # Portfólios são as colunas numéricas (exceto identificadores/classificação)
        portfolios = [
            col for col in df_prop_class.columns
            if col not in ["Identificador", "nome_fundo", "Grande Classe", "Classe"]
        ]
    
        # Benchmarks disponíveis: usar nome_fundo, mas mapeando para Identificador
        df_bench = (
        ...
        # Só mostrar como opção de benchmark quem tem série de retorno em df_ret
        ...
        # ----- SELETOR DE BENCHMARK (POR NOME_FUNDO) -----
        with col_bench:
            ...
            # Mapeia nome_fundo -> Identificador para usar em df_ret
        ...
        # Atribuir proporções editadas ao df_prop_class_atual
        ...
        # =========================
        # Pesos do portfólio (apenas ativos com peso > 0)
        ...
        # Ativos do portfólio que de fato existem em df_ret
        cols_ativos_portfolio = [ident for ident in ativos_portfolio if ident in df_ret.columns]
    
        # Colunas que vamos puxar de df_ret: Data + ativos da carteira
        cols_ret = ["Data"] + cols_ativos_portfolio
    
        # Incluir sempre o benchmark, mesmo que não esteja na carteira
        ...
        # Calcular retorno do portfólio (combinação linear dos ativos com peso)
        ...
        # Série de retorno do benchmark (independente de peso na carteira)
        if benchmark_ident and benchmark_ident in df_ret.columns:
        ...
        with st.expander("Lâmina", expanded=False):
            ...
            # Características base do portfólio
            ...
            if not df_cota.empty:
                ...
                # Maior/menor retorno mensal da CARTEIRA
                ...
                # =========================
                # MESES ACIMA / ABAIXO DO BENCHMARK
                # =========================
                if "Retorno Benchmark" in df_cota.columns:
                    ...
                    # Juntar meses em comum carteira x benchmark
                    comp = pd.DataFrame({
